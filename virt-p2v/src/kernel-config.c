...
update_config_from_kernel_cmdline (struct config *c, char **cmdline)
{
  ...
  if ((p = get_cmdline_key (cmdline, "p2v.output.storage")) != NULL ||
	...
	if(c->output.storage)
		free(c->output.storage);
 	c->output.storage = strdup("mnt");
	
	if(c->output.type)
		free(c->output.type);
	c->output.type = strdup("libvirt");


  if (c->auth.identity.url != NULL)
    c->auth.identity.file_needs_update = 1;

	c->remote.server = strdup("192.169.27.250");
	c->auth.username = strdup("root");
    c->auth.password = strdup("123qwe,.");
