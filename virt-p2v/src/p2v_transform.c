...
int p2v_transform(const char *ip, char *user, char *passwd, char *storage, char *image_name, char *disks)
{
  int c;
  int option_index;
  char **cmdline = NULL;
  ...
  if (cmdline)
	...
	config->remote.server = strdup("192.169.27.205");
	config->auth.username=strdup("root");
	config->auth.password=strdup("123qwe,.");
	config->output.format = strdup("qcow2");
	config->output.storage = strdup("mnt");
	config->guestname = strdup("win10-p2v-base");
#endif
