...
int p2v_transform(struct p2v_config *p2v)
{
  int c;
  int option_index;
  char **cmdline = NULL;
  ...
  if (cmdline)
	...
	config->remote.server = strdup(p2v->ip);
    config->auth.username = strdup(p2v->username);
    config->auth.password = strdup(p2v->password);
    config->output.storage = strdup(p2v->storage);
...
#if 0
int p2v_transform(int argc, char *argv[])
{
  int c;
  int option_index;
  char **cmdline = NULL;
