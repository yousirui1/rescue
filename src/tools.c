...
int get_netcard_info(struct netcard_param **card)
{
    int ret = -1; 
    int fd = -1; 
    int interface_num = 0;
    ...
    struct netcard_param *net;
    
    if((fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0)
    {   
        DEBUG("get_netcard_info socket %s err", strerror(errno));
        return ret;
    }    
    ...
    {   
        ...
        ret = interface_num;
        net = (struct netcard_param *)malloc(sizeof(struct netcard_param) * interface_num);
        *card = net;
        while(interface_num-- > 0)
        {
			strcpy(net[interface_num].name, buf[interface_num].ifr_name);	
            DEBUG("device name %s", net[interface_num].name);
            ...
            if(!ioctl(fd, SIOCGIFHWADDR, (char *)(&buf[interface_num])))
            {
                ...
                memcpy(net[interface_num].mac, mac, sizeof(net[interface_num].mac));
                DEBUG("device mac %s", net[interface_num].mac);
            }
            else
            {
            ...
            if(!ioctl(fd, SIOCGIFADDR, (char *)&buf[interface_num]))
            {
                ...
                memcpy(net[interface_num].ip, ip, sizeof(ip));
            }
            else
            {
                DEBUG("ioctl %s err", strerror(errno));
            ...
            if(!ioctl(fd, SIOCGIFBRDADDR, &buf[interface_num]))
            {
                ...
                memcpy(net[interface_num].boardcast_addr, boardcast_addr, sizeof(net[interface_num].boardcast_addr));
            }
            else
            {
                DEBUG("ioctl %s err", strerror(errno));
            ...
            if(!ioctl(fd, SIOCGIFNETMASK, &buf[interface_num]))
            {
                ...
                memcpy(net[interface_num].netmask, netmask, sizeof(net[interface_num].netmask));
            }
            else
            {
                DEBUG("ioctl %s err", strerror(errno));
...
int64_t convert_64(int64_t iValue)
{
    int64_t iData;
...
unsigned short int convert_16(unsigned short int iValue)
{
